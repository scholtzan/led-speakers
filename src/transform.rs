...
impl AudioTransformer {
    ...
    pub fn start(&mut self) {
        ...
        self.handle = Some(thread::spawn(move || {
            let audio = AudioStream::new("led speakers".to_string(), source, buffer_size);
            ...
            let mut stream_buf = BytesMut::with_capacity(target_bytes_per_frame * 6 + 32 * bins);
    ...
    fn frequency_magnitudes(
        ...
        for n in 0..total_bands {
            ...
            while cutoff_freq <= upper_cutoff_freq[n] as usize
                && (cutoff_freq as usize) < total_bands
            {
                cutoff_freq += 1;
